# Hospedagem num servidor próprio (guardada para quando escalar)

> **Não é o caminho em uso.** Hoje o sistema roda de graça no Render e no Supabase — o
> guia é o [hospedagem.md](./hospedagem.md), e o porquê está no
> [ADR 0013](./adr/0013-hospedagem-gratuita-em-pecas.md). Este fica para quando valer
> pagar um servidor: o sistema inteiro numa máquina só, com banco, HTTPS e cópia todo
> dia, e sem os limites do plano gratuito.
>
> Ele foi escrito e ensaiado para a Oracle Cloud, que ficou para trás. Os passos 1, 2, 4
> e 5 são do painel da Oracle; num servidor pago (uma VPS com Ubuntu 24.04, como as da
> Hostinger), eles viram "contratar a máquina, com um usuário `ubuntu` que entra pela
> chave do passo 3 e usa `sudo` sem senha" e "abrir as portas 80 e 443 no firewall do
> provedor". O passo 3 e os do 6 em diante valem para qualquer servidor.
>
> Para levar os dados do Supabase, o caminho é o do
> [recomeço num servidor novo](#quando-algo-dá-errado), com o endereço do Supabase no
> lugar do `BACKUP_NEON_URL` e só os schemas do sistema (`--schema=public
> --schema=drizzle`: o resto daquele banco é do Supabase). Ensaie antes num banco de
> teste — esse caminho ainda não foi feito.

Este é o passo a passo para pôr o sistema na internet num servidor, com endereço
`https://` e atualização automática: toda mudança que entra no `main` vai para o ar em
cerca de cinco minutos, sem você ligar nada no seu computador. A decisão e o porquê de
cada peça estão no [ADR 0010](./adr/0010-hospedagem-gratuita.md).
...
dias ociosa — processador, rede e memória abaixo de 20%. O servidor tinha uma reserva de
memória para não se encaixar na regra (ADR 0012), que saiu junto com a Oracle (ADR 0013)
e está no histórico do git, em `servidor/reserva.mjs`. Se a máquina for recolhida, os
dados estão na cópia do Neon (passo 10), e o [recomeço](#quando-algo-dá-errado) leva uns
20 minutos.
...
programa pedindo o tempo todo. Foi esse o motivo de a Oracle ficar para trás (ADR 0013).
...
> segundo em que ela abre, e não é motivo para fazer upgrade (passo 2). O pedido
> automático pela API, que tentava uma vez por minuto, está no histórico do git
> (`servidor/criar-maquina.py`, ADR 0013).

## Passo 5 — Abrir as portas 80 e 443 na rede da Oracle

A rede da Oracle deixa entrar só o SSH, e o site precisa das portas da web:
...
**Criar a máquina dá "Out of capacity".** Ver o passo 4.2.
